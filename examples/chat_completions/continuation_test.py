...
def setup_colored_logging():
    ...
    try:
        ...
        coloredlogs.install(
            level=logging.INFO, logger=logger, fmt="[%(levelname)s] %(message)s"
        )
    except ImportError:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Test conversation continuation RAG skipping"
    )
...
def send_chat_completion_request(
    host: str, model: str, messages: List[Dict[str, Any]], verbose: bool = False
) -> Dict[str, Any]:
...
def main():
    ...
    messages = [
        ...
        {"role": "user", "content": "Tell me about Deepgram's Nova-2 model."},
    ]

    # First response should include RAG
    assistant_message = send_chat_completion_request(
        args.host, args.model, messages, args.verbose
    )
    ...
    messages.append(
        {"role": "user", "content": "How does it compare to the previous Nova-1 model?"}
    )

    # Send the follow-up request
    time.sleep(1)  # Small delay for readability in logs
    follow_up_message = send_chat_completion_request(
        args.host, args.model, messages, args.verbose
    )
