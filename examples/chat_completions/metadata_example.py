...
Unless they ask for a specific code language, use a cURL example.
    ...
    parser.add_argument("--user", 
                       ...
                       help="User message")
    parser.add_argument("--stream", action="store_true", help="Enable streaming mode (note: metadata not available in streaming mode)")
    ...
    if args.stream:
        print(f"{Fore.YELLOW}[WARNING] Metadata is not available in streaming mode{Style.RESET_ALL}")
...
def print_metadata_report(response_data: Dict[str, Any], total_request_duration: float) -> None:
    ...
    if "usage" in response_data:
        ...
        print(f"  • API Token Usage: {usage.get('prompt_tokens', 0)} prompt + "
    ...
    if operations:
        ...
        for op_type, ops in op_by_type.items():
            ...
            for i, op in enumerate(ops, 1):
                ...
                print(f"     • Latency: {latency if latency == 'N/A' else f'{latency:.2f}ms'}")
